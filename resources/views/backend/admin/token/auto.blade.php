...
(function($) {
    ...
    $("input[name=client_name]").on('input', function(e){
        var valid = true;
        var clientErrMessage = "";
        var clientName = $(this).val();

        if (clientName === '') {
            clientErrMessage = "The Name field is required!";
            valid = false;
        } else if (!isNaN(clientName)) {
            clientErrMessage = "The Name is incorrect!";
            valid = false;
        }

        if (!valid && clientErrMessage.length > 0) {
            $('.modal button[type=submit]').addClass('hidden');
        } else {
            $(this).next().html(" ");
            $('.modal button[type=submit]').removeClass('hidden');
        }
        $(this).next().html(clientErrMessage);
    });
