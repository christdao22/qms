    ...
    <div class="panel-body">
        ...
        {{ Form::open(['url' => 'admin/setting', 'files' => true, 'class'=>'col-md-7 col-sm-8']) }}
            ...
            <div class="form-group @error('city') has-error @enderror">
                ...
                <textarea name="city" id="city" class="form-control" placeholder="City">{{ old('city')?old(
                'city'):$setting->city }}</textarea>
                <span class="text-danger">{{ $errors->first('city') }}</span>
            </div>

            <div class="form-group @error('office') has-error @enderror">
                <label for="office">Office </label>
                <input name="office" id="office" class="form-control" placeholder="Office" value="{{ old('office')?old(
                'office'):$setting->office }}"/>
                <span class="text-danger">{{ $errors->first('office') }}</span>
            </div>

            <div class="form-group @error('sds_fullname') has-error @enderror">
                <label for="sds_fullname">Schools Division Superintendent </label>
                <input name="sds_fullname" id="sds_fullname" class="form-control text-capitalize" placeholder="Schools Division Superintendent" value="{{ old('sds_fullname')?old(
                'sds_fullname'):$setting->sds_fullname }}"/>
                <span class="text-danger">{{ $errors->first('sds_fullname') }}</span>
            </div>
            ...
            <div class="form-group @error('deped_seal') has-error @enderror">
                <label for="deped_seal">Deped Seal </label>
                <img src="{{ asset((session('deped_seal')?session('deped_seal'):$setting->deped_seal)) }}" alt="deped_seal" class="img-thubnail thumbnail" width="50" height="50">
                <input type="hidden" name="old_deped_seal" value="{{ ((session('deped_seal') != null) ? session('deped_seal') : $setting->deped_seal) }}">
                <input type="file" name="deped_seal" id="deped_seal" class="form-control">
                <span class="text-danger">{{ $errors->first('deped_seal') }}</span>
