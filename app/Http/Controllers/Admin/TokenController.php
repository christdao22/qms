...
class TokenController extends Controller
{
    ...
    public function tokenAutoView()
    {
        ...
        return view('backend.admin.token.auto', compact('display', 'departmentList', 'keyList', 'schools'));
    }
    ...
    public function showForm()
    {
        ...
        $officers = User::select(DB::raw('CONCAT(firstname, " ", lastname) as name'), 'id')
            ...
            ->pluck('name', 'id');
        return view('backend.admin.token.manual', compact('display', 'counters', 'departments','officers', 'schools' ));
    }
                    ...
                    if (!empty($search['value'])) {
                        ...
                        else
                        {
                            ...
                            $query->where('token_no', 'LIKE',"%{$search['value']}%")
                                ->orWhere('client_name', 'LIKE',"%{$search['value']}%")
        ...
        if(!empty($tokens))
        {
            ...
            foreach ($tokens as $token)
            {
                ...
                $data[] = [
                    ...
                    'client_name'    => $token->client_name. "<br/>" .(!empty($token->client)?("(<a href='".url("admin/user/view/{$token->client->id}")."'>".$token->client->firstname." ". $token->client->lastname."</a>)"):null),
    ...
    public function getCounter($department_id = null) {
        if($department_id == null) return;

        $data = TokenSetting::where('token_setting.department_id', $department_id)
            ->leftJoin('counter', 'token_setting.counter_id', '=', 'counter.id')
            ->leftJoin('user', 'token_setting.user_id', '=', 'user.id')
            ->get(['counter_id', 'counter.name as counter_name', 'user_id', 'user.firstname', 'user.lastname']);

        return response()->json($data);
    }

    public function getOfficer($counter_id = null) {
        if($counter_id == null) return;

        $data = TokenSetting::where('token_setting.counter_id', $counter_id)
            ->leftJoin('user', 'token_setting.user_id', '=', 'user.id')
            ->get(['user_id', 'user.firstname', 'user.lastname']);
